...
public class bookingLab {
    ...
    public TicketDetails findTicket(int ticketId) {
        Cursor cursor = mDatabase.query(
                DatabaseSchma.BookingTable.NAME,
                null,
                DatabaseSchma.BookingTable.Cols.ticketId + " = ? ",
        ...
        if (cursor != null) {
            ...
            try {
                return new TicketDetails(
                        C2I(cursor.getString(cursor.getColumnIndex(DatabaseSchma.BookingTable.Cols.ticketId))),
                        cursor.getString(cursor.getColumnIndex(DatabaseSchma.BookingTable.Cols.toStation)),
                        cursor.getString(cursor.getColumnIndex(DatabaseSchma.BookingTable.Cols.fromStation)),
                        cursor.getString(cursor.getColumnIndex(DatabaseSchma.BookingTable.Cols.journyType)),
                        cursor.getString(cursor.getColumnIndex(DatabaseSchma.BookingTable.Cols.date)));
            } catch (Exception ex) {
                flage = true;
            }
        } else {
    ...
    public void deletTicket(int ticketId) {
        ...
        int n = mDatabase.delete(DatabaseSchma.BookingTable.NAME, DatabaseSchma.BookingTable.Cols.ticketId + "= ?", new String[]{String.valueOf(ticketId)});
        mDatabase.close();
        if (n > -1) {
